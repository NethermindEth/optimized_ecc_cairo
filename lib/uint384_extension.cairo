...
namespace uint384_extension_lib {
    ...
    func unit256_mul_split(x0: felt, x1: felt, x2: felt, x3: felt,
			   y0: felt, y1: felt, y2: felt, y3: felt) ->
      (z0: felt, z2: felt, z4: felt, z6: felt) {
        return (x0*y0 + (x1*y0 + x0*y1) * HALF_SHIFT,
		x2*y0 + x1*y1 + x0*y2 + (x3*y0 + x2*y1 + x1*y2 + x0*y3) * HALF_SHIFT,
		x3*y1 + x2*y2 + x1*y3 + (x3*y2 + x2*y3) * HALF_SHIFT,
		x3*y3);
    }
    
    func unit256_mul_splitb(x0: felt, x1: felt, x2: felt, x3: felt,
			   y0: felt, y1: felt) ->
    ...
    func Toom25_interp(z0: felt, z4: felt, p1: felt, pm1: felt) -> (
        z1: felt, z2: felt
    ) {
        alloc_locals;
	const HALF = 1/2;
    ...
    func mul_uint768_by_uint384_Toom25{range_check_ptr}(a: Uint768, b: Uint384) -> (low: Uint768, high: Uint384) {
        alloc_locals;
        let (a0, a1) = uint384_lib.split_64(a.d0);
        let (a2, a3) = uint384_lib.split_64(a.d1);
        let (a4, a5) = uint384_lib.split_64(a.d2);
